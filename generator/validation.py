...
def validate_enums(file_path, enums):
        ...
        for name, enum in enums.items():
            ...
            if len(set(enum.values())) != len(enum.values()):
                raise CerealPackException(file_path, 'enum "{}" values are not unique'.format(name))
            if 0 not in map(lambda v: int(v), enum.values()):
                raise CerealPackException(file_path, 'enum "{}" must contain a `0` value, this is the default'.format(name))
